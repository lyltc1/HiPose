...
"""
"""
modified by Yongzhi Su. Added combination of multiple symmetry identical matrix
"""
...
def modified_gt_for_symmetry(rot_pose, tra_pose, model_info):
    ...
    if 'symmetries_continuous' in model_info and 'symmetries_discrete' in model_info:
        ...
        n_sym_matrix = len(model_info['symmetries_discrete'])

        for i in range(n_sym_matrix):
            sym = model_info['symmetries_discrete'][i]
            sym_4x4 = np.reshape(sym, (4, 4))
            R = sym_4x4[:3, :3]
            t = sym_4x4[:3, 3].reshape((3, 1))
            trans_discs.append({'R': R, 't': t})
            for j in range(i+1, n_sym_matrix):
                sym_next = model_info['symmetries_discrete'][j]
                sym_next_4x4 = np.reshape(sym_next, (4, 4))
                sym_combined = sym_4x4.dot(sym_next_4x4)
                R_combined = sym_combined[:3, :3]
                t_combined = sym_combined[:3, 3].reshape((3, 1))
                trans_discs.append({'R': R_combined, 't': t_combined})
