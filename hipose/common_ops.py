...
def from_output_to_class_mask_with_uncertainty(pred_mask_prob, thershold=0.5):
    # the range of score is [0, 1], 1 means the network is very sure with the result
    activation_function = torch.nn.Sigmoid()
    pred_mask_prob = activation_function(pred_mask_prob)
    pred_mask_prob = pred_mask_prob.detach().cpu().numpy()
    pred_mask = np.zeros(pred_mask_prob.shape)
    pred_mask[pred_mask_prob>thershold] = 1.
    pred_mask_score = 4 * np.square(pred_mask_prob - 0.5)
    return pred_mask, pred_mask_score

def from_output_to_class_binary_code_with_uncertainty(pred_code_prob, BinaryCode_Loss_Type, thershold=0.5, divided_num_each_interation=2, binary_code_length=16):
    if BinaryCode_Loss_Type == "BCE" or BinaryCode_Loss_Type == "L1":   
        activation_function = torch.nn.Sigmoid()
        pred_code_prob = activation_function(pred_code_prob)
        pred_code_prob = pred_code_prob.detach().cpu().numpy()
        pred_code = np.zeros(pred_code_prob.shape)
        pred_code[pred_code_prob>thershold] = 1.
        pred_code_score = 4 * np.square(pred_code_prob - 0.5)
    else:
        raise NotImplementedError
    return pred_code, pred_code_score
